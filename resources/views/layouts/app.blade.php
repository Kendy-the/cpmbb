    ...
    <div class="flex justify-center items-center mx-auto max-w-screen-xl">
        <header class="header border-b" id="header">
            <div class="w-full justify-between flex items-center">
                ...
                {{-- first part --}}
                <div class="flex w-2/12 justify-start items-center relative right-4">
                ...
                {{-- form --}}
                <form action="/account/search" method="post" class="w-7/12 hidden lg:flex items-center">
                    <i class='bx bx-search text-lg'></i>
                    <input type="search" name="" id="" placeholder="Search"
                        class="w-full rounded ps-8 h-9 hidden lg:inline-block border-[#DFEAF2] focus:outline focus:outline-violet-400">
                </form>
                        ...
                        <div>
                            <a href="/account/profile" data-popover-target="popover-user-profile">
                                @if (isset($user) && is_null($user->image))
                                    <div
                                        class="bg-violet-100 rounded-full w-13 h-13 me-3 flex justify-center items-center">
                                        {{ isset($user) ? Str::upper(Str::substr($user->first_name, 0, 1)) . ' ' . Str::upper(Str::substr($user->last_name, 0, 1)) : 'JD' }}
                                    </div>
                                @else
                                    <img src="{{ isset($user) ? $user->imageUrl() : '' }}"
                                        class="rounded-full w-12 h-12 me-3 border border-gray-300 object-cover">
                                @endif
                            </a>

                            {{-- pop over data --}}
                            <div data-popover id="popover-user-profile" role="tooltip"
                                class="absolute p-1 z-10 invisible inline-block text-sm text-gray-500 transition-opacity duration-300 bg-white border border-gray-200 rounded-lg shadow-xs opacity-0 hover:bg-violet-400 hover:text-white active:bg-violet-500">
